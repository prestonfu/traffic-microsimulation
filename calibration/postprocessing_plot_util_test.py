...
from utils.aimsun_input_utils import FlowRealData, AimsunFlowRealDataSet
from calibration.postprocessing_plot_util import (
...
AIMSUN_DETECTOR_EXTERNAL_ID_LIST = [
    "flow_detector_201901",
    "flow_detector_201902",
    "flow_detector_403829",
    "flow_detector_403928",
]
...
class TestAllMethods(unittest.TestCase):
    ...
    def test_convert_flow_per_time_to_list(self):
        ...
        self.assertTrue(all(real_flow_list == np.arange(count).reshape(-1, 1)))
        (
            city_real_flow_list,
        ...
        self.assertTrue(
            int(np.average(city_real_flow_list) + np.average(pems_real_flow_list))
            == count - 1
        )
    ...
    def test_get_linear_regression(self):
        ...
        arr_2 = arr_1
        temp_slope, temp_intercept, temp_r_sq, max_val_1, _ = get_linear_regression(
            arr_1, arr_2, False
        )
        ...
        self.assertTrue(
            (slope_1, intercept_1, r_sq_1, max_val_1) == (1, 0, 1.0, sample_size - 1)
        )

        expected_intercept = 3
        expected_slope = 2
        arr_3 = np.array(
            [expected_slope * x + expected_intercept for x in range(sample_size)]
        ).reshape(-1, 1)
        (
            temp_slope,
            temp_intercept,
        ...
        self.assertTrue(
            (slope_2, intercept_2, r_sq_2, max_val_2)
            == (expected_slope, expected_intercept, 1.0, sample_size - 1)
        )
        for i, y_val in enumerate(yhat_2):
            self.assertTrue(int(round(y_val[0], decimal_error_bound)), arr_3[i][0])

        temp_slope, temp_intercept, temp_r_sq, max_val_3, _ = get_linear_regression(
            arr_1, arr_3, True
        )
        ...
        self.assertTrue(
            (slope_3, intercept_3, max_val_3) == (expected_slope, 0, sample_size - 1)
        )
    ...
    def test_process_real_flow_data(self):
        ...
        flow_real_data_list = []
        count = 0
        for _, detector_external_id in enumerate(REAL_DETECTOR_EXTERNAL_ID_LIST):
            flow_real_data_obj = FlowRealData()
            flow_real_data_obj.external_id = detector_external_id
            flow_data_dict = {}
            for time in TIME_LIST:
                flow_data_dict[
                    datetime.timedelta(hours=time.hour, minutes=(time.minute + 15))
                ] = count
                count += 1
        ...
        self.assertTrue(detector_external_id_list == AIMSUN_DETECTOR_EXTERNAL_ID_LIST)
        self.assertTrue(list(real_flow_per_time.keys()) == TIME_LIST)
        self.assertTrue(
            list(real_flow_per_time[TIME_LIST[0]].keys())
            == AIMSUN_DETECTOR_EXTERNAL_ID_LIST
        )
        self.assertTrue(
            list(real_flow_per_detector.keys()) == AIMSUN_DETECTOR_EXTERNAL_ID_LIST
        )
